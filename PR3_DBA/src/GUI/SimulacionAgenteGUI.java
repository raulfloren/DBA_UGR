...
public class SimulacionAgenteGUI extends JFrame {
    ...
    // --- NUEVO: Objeto para sincronizar el paso a paso ---
    ...
    private void inicializarComponentes(String textoInicial) {
        ...
        // Si quieres "Pantalla completa real" (sin bordes ni barra de título), descomenta esto:
        // setUndecorated(true);
         ...
         */
        JPanel panelBoton = new JPanel(new GridBagLayout()); // Usamos GridBagLayout para apilar
        GridBagConstraints gbcBoton = new GridBagConstraints();
        gbcBoton.fill = GridBagConstraints.HORIZONTAL; // Que tengan el mismo ancho
        gbcBoton.insets = new Insets(5, 0, 5, 0); // Espacio vertical

        // Botón Siguiente Paso
        siguienteButton = new JButton("Siguiente Paso ▶");
        siguienteButton.setBackground(new Color(200, 255, 200));
        siguienteButton.addActionListener(e -> {
            // Notificar al hilo que espera
            synchronized (lock) {
                lock.notify();
            }
        });
        ...
        gbcBoton.gridx = 0;
        gbcBoton.gridy = 0; // Fila superior
        panelBoton.add(siguienteButton, gbcBoton);

        // Botón Cerrar
        JButton cerrarButton = new JButton("Cerrar");
        cerrarButton.addActionListener(e -> dispose());

        gbcBoton.gridy = 1; // Fila inferior
        panelBoton.add(cerrarButton, gbcBoton);
    ...
    /**
     * Helper method to create and add a single row (Image and Text) to the grid
     * panel.
     */
    ...
    private void dibujarMatriz(Graphics g) {
        ...
        // 3. CAMBIO: Obtener dimensiones dinámicas del panel
        int anchoPanel = panelMatriz.getWidth();
        int altoPanel = panelMatriz.getHeight();

        // Calcular el tamaño de celda basándose en el espacio disponible
        // Restamos un pequeño margen (ej. 20px) para que no quede pegado a los bordes
        int anchoCelda = Math.min((anchoPanel) / columnas, (altoPanel) / filas);
        int altoCelda = anchoCelda; // Queremos celdas cuadradas

        // Calcular offsets para centrar la matriz en la pantalla
        int offsetX = (anchoPanel - (anchoCelda * columnas)) / 2;
        int offsetY = (altoPanel - (altoCelda * filas)) / 2;
        ...
        for (int i = 0; i < filas; i++) {
            for (int j = 0; j < columnas; j++) {
                ...
                // Coordenadas de dibujo usando los nuevos offsets
                int x = offsetX + j * anchoCelda;
                int y = offsetY + i * altoCelda;

                if (imagen != null) {
                    g.drawImage(imagen, x, y, anchoCelda, altoCelda, this);
                } else {
                    switch (valor) {
                        case MURO ->
                            g.setColor(Color.DARK_GRAY);
                ...
                if (valor == AGENTE) {
                    Image imgAgente = imagenesAgente.get(direccionAgente);
                    if (imgAgente != null) {
                        g.drawImage(imgAgente, x, y, anchoCelda, altoCelda, this);
                    } else {
                        g.setColor(Color.BLUE);
                        g.fillOval(x + anchoCelda / 4, y + altoCelda / 4, anchoCelda / 2, altoCelda / 2);
                    }
                }
    ...
    /**
     * Actualiza la imagen y el mensaje de estado en una fila específica del
     * panel derecho.
     *
     * @param row La fila a actualizar (1 a 4).
     * @param mensaje El texto de estado a mostrar.
     * @param imagenKey La clave de la imagen a mostrar (ej. AGENTE, SANTA,
     * etc.).
     */
    ...
    /**
     * MÉTODO DE BLOQUEO: Detiene la ejecución hasta que se pulse el botón.
     */
    public void esperarInputUsuario() {
        try {
            synchronized (lock) {
                lock.wait();
            }
